...
void initialization_card::loading_card(SDL_Surface *SCREEN)
{
...
// Завантаження зображень з файлів на поверхні SDL_Surface.
void initialization_card::InitImages(void)
{
	//задний фон логотипу
...
/* --------------------------------------------------------------------------------------*/
// завантажуємо музику
void initialization_card::load_audio(void)
{
	Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 4096);
	// завантажуємо музику
	//music = Mix_LoadMUS("resources/audio/logo.wav");
}
int initialization_card::load_card(SDL_Surface *SCREEN)
{
	...
	while (!logo_exit)
	{
		if (logo_run)
		{
			...
			while (enter)
			{
				...
				{
					text.Draw_Text(250, 200, 250, 38, font, number_s_account, 0, 0);
				}
				text.Draw_Text(20, 100, 800, 38, font, "Вставте карту номерном вверх, пожалуйста", 0, 0);
				text.Draw_Text(100, 140, 800, 38, font, "(введите 12 цифр номера карты)", 0, 0);
				if (errors == true)
					text.Draw_Text(100, 50, 800, 38, font, "(Такой счет не активный)", 0, 0);
				glColor4f(1, 1, 1, 1);
...
// DrawBG_logo () відображає задній фон на екран.
void initialization_card::DrawBG_logo(SDL_Surface *SCREEN)
{
	DrawIMG(bc_logo_S, 0, 0, SCREEN);
}
// DrawIMG () малює зображення на екран в заданій позиції.
void initialization_card::DrawIMG(SDL_Surface *img, int x, int y, SDL_Surface *SCREEN)
{
	dstrect.x = x;
	dstrect.y = y;
...
void initialization_card::DrawScene_logo(SDL_Surface *screen)
{
	SDL_GL_SwapBuffers();
